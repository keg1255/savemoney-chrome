 ...
 */
export function matchString(s, quotation = '"', escape = "\\") {
	s = s.trimStart();
	if (!s) return {value: "", next: ""};
	if (quotation.indexOf(s[0]) < 0) {
		// 不是引号开头
		let m = /\s/.exec(s);
		if (m) {
	...
	let c = s[0];
	s = s.slice(1);
	let m = new RegExp(`(?<!${escape})${c}`).exec(s);
	let value, next;
	if (m) {
		value = s.slice(1, m.index);
		next = s.slice(m.index + 1);
	} else {
		value = s.slice(1);
		next = "";
	}
	value = value.replace(new RegExp(`${escape}${c}`, "g"), c);
	return {value, next};
}
